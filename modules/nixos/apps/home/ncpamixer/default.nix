...
with lib;
with lib.plusultra;
let cfg = config.plusultra.apps.home.ncpamixer;
in
{
  options.plusultra.apps.home.ncpamixer = with types; {
    enable = mkBoolOpt false "Whether or not to enable ncpamixer.";
  };

  config = mkIf cfg.enable {
    # environment.systemPackages = with pkgs; [ ncpamixer ];
    plusultra.home.extraOptions.home.packages = with pkgs; [ ncpamixer ];
    plusultra.desktop.Hyprland.ncpamixer = "bind = $mainMod, F5, exec, foot -o colors.alpha=0.5 --app-id floating,ncpamixer -e ncpamixer";
    plusultra.home.configFile."pulsemixer.cfg".text = ''
"theme" = "default"
